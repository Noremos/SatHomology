...
inline MLSettings mkSettingsType()
{
	MLSettings settings;
...
inline Barscalar lerp(double t)
{
	// if (t < 0)
	// 	t = 0;
...
inline bc::barstruct getConstr(const MLSettings& setting)
{
...
inline BackSize getSrcFromInput(InOutLayer iol, BackImage& out, std::optional<BackSize> newSize = {})
{
...
inline RasterLayer* genOutputFromInput(InOutLayer iol, BackImage& out, std::optional<BackSize> newSize = {})
{
